...
| The machine in one declaration, queued, on a shared workqueue | both machines are `mtl::zephyr::StateMachine`: each brings its timeout timer and its event buffer, and both drain on one `mtl::zephyr::WorkQueue<2048, 5> fsm_queue` handed over after the table (without it each would own a thread). `monitor` names its table with `table_for<sensor::sensor_table>` because the table is a template over the deduced observers; the LED machine is a class member, where deduction is unavailable, hence `StateMachineOnSharedWorkqueue<...>`. The button ISR, the `k_timer` expiries, the sensor's work items on the system workqueue and `LedController` from inside the sensor machine's hook all just `process()` - the observers report through `reportReading()` and friends instead of holding a machine pointer |
